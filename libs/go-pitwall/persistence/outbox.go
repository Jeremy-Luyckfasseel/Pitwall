...
// OutboxRow is one durable outgoing event awaiting (or past) publication. It expects
// an `outbox` table with columns: id, routing_key, payload, status, attempts,
// last_error, created_at, sent_at (the service supplies the DDL in its migrations).
type OutboxRow struct {
	ID         string // = envelope id
	RoutingKey string // = envelope type; the key published on the service's exchange
...
// inside the producer's own transaction (atomic with the domain write); the other
// methods are driven by the background relay.
...
// Enqueue inserts a pending outbox row using the caller's transaction, so the row
// commits together with whatever domain state the same tx wrote. The row is always
...
// FetchPending returns up to limit pending rows, oldest-first (created_at), so the
// relay publishes in production order. The id tie-breaker keeps the order stable when
...
// MarkSent flips a row to sent after a confirmed broker ack (sent only after ack).
...
// MarkQuarantined terminally quarantines a row that could not be validated against
// /contract: it is never published and never retried. This is a producer-side
// quarantine (a local status), distinct from the consumer-side RabbitMQ DLQ/parking
// topology.
...
// RecordFailure notes a transient publish failure (broker unreachable / nack): the
// row stays pending and is retried on a later tick.
