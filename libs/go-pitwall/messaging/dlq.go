...
// Consumer-side DLQ topology. The blueprint mandates: a poison or transient-failing
// message dead-letters to the consumer's DLX, is retried from a single `.retry` queue
// with per-hop exponential backoff (per-message TTL, dead-lettering back to the work
// queue), and on exceeding the delivery-count cap lands terminally in `.parking` (+ a
// Control-Room alert). Classic queues (Q&A Round 27). Names follow AR10: DLX
// `<consumer>.dlx`, retry `<consumer>.<purpose>.retry`, parking
// `<consumer>.<purpose>.parking`. The DLX name itself is the SERVICE's topology
// (ConsumerOptions.DLXExchange); the library carries only the mechanics.
const (
	// Routing keys on the DLX. retry → the retry queue; redeliver → the work queue
	// (where the retry queue dead-letters a message back after its TTL); park → the
	// terminal parking queue.
...
// (`<work>.retry`, `<work>.parking`) so the topology is consistent.
...
// RetryCount reports how many times this delivery has already been retried (header
// retryCountHeader; absent ⇒ 0).
...
// integer header back as int32 / int64 / int. Any missing or non-integer value is
...
// buildDLXPublishing builds the persistent message for a DLX publish. For a RETRY it
// carries a per-message Expiration (the exponential backoff TTL, ms) and the
// incremented retry-count header; for a PARK it omits Expiration and stamps the reason
// header. Pure (no I/O) so it is unit-tested without a broker.
...
// DeclareDLQTopology declares the full consumer-side DLQ topology for the given work
// queue and binds the work queue to its source exchange. It supersedes
// DeclareConsumerQueue for the production path: the work queue is declared WITH
// dead-letter args (a one-time, self-healing migration from an arg-less queue — see
...
func (b *Bus) DeclareDLQTopology(opts ConsumerOptions) error {
	...
	if err := ch.ExchangeDeclare(opts.DLXExchange, "direct", true, false, false, false, nil); err != nil {
		return err
	}
	...
	if _, err := ch.QueueDeclare(retryQueue, true, false, false, false, amqp.Table{
		"x-dead-letter-exchange":    opts.DLXExchange,
	...
	// Work queue WITH dead-letter args (safety net: an unmodelled reject parks rather
	// than drops). Classic queue args are immutable, so redeclaring an arg-less queue
	// needs a delete+redeclare — handled resiliently.
	...
	for _, bind := range []struct{ key, queue string }{
		...
		if err := ch.QueueBind(bind.queue, bind.key, opts.DLXExchange, false, nil); err != nil {
			return err
		}
	}
	// Work queue ← source exchange, on every routing key (one queue preserves publish
	// order across lap.recorded + session.*).
	...
	// Handler goroutine — guard the write so the two never race.
	b.mu.Lock()
	b.dlx = opts.DLXExchange
	b.mu.Unlock()
	return ch.Qos(opts.Prefetch, 0, false)
}

// declareWorkQueueResilient declares the work queue with the given args, healing the
// classic-queue immutable-args constraint: if an arg-less queue of the same name
// already exists, declare-with-args raises a 406 PRECONDITION_FAILED that closes the
// channel — so the probe runs on a throwaway channel, and on 406 we delete the old
// queue and redeclare with args on a fresh channel. Fresh brokers simply declare. The
// main channel (b.ch) is never poisoned by the probe.
//
// The delete is **empty-only** (`ifEmpty=true`): we never drop a buffered-but-
// unconsumed message during the one-time migration, and we do NOT assume any peer will
// re-emit it (golden rule). If the stale queue still holds messages, we refuse loudly
// so an operator drains it first — "never silently dropped" over convenience.
func (b *Bus) declareWorkQueueResilient(name string, args amqp.Table) error {
	...
	// queue to one carrying the DLX args by delete + redeclare on a fresh channel — but
	// only if it is EMPTY, so no in-flight message is ever lost.
...
// isPreconditionFailed reports whether err is an AMQP 406 (inequivalent queue args)
// channel exception.
...
// PublishToDLX publishes body to the consumer's DLX with the given routing key. For a
// retry, expirationMs is the per-hop backoff TTL and retryCount the incremented hop
// count; for a park, expirationMs is 0 and parkReason records why. Manual-ack
// discipline: the caller acks the ORIGINAL delivery only after this republish succeeds.
