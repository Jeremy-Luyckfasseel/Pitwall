// Package hygiene holds source-level guard tests for the shared blueprint library.
// It has no production code. Mirrors the per-service hygiene guard so the extracted
// mechanics are held to the same structured-logging rule (no bare fmt.Print*/println
// to stdout — the logging package is the only sink; blueprint §Observability, NFR20).
package hygiene
...
// TestNoBarePrintStatements inspects the AST (not text) of every production .go file
// in the module so doc comments and string literals never false-positive.
func TestNoBarePrintStatements(t *testing.T) {
	_, thisFile, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("cannot resolve test file path")
	}
	// hygiene lives at libs/go-pitwall/hygiene; the module root is one level up.
