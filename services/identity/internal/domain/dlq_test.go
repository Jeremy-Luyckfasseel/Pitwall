...
var pol = domain.DLQPolicy{MaxAttempts: 5, BaseMs: 1000, Multiplier: 2, MaxMs: 60000}
...
func TestNextRetry_BackoffThenPark(t *testing.T) {
	...
	for _, c := range cases {
		got := domain.NextRetry(c.prior, pol)
...
func TestNextRetry_DelayClampedToMax(t *testing.T) {
	p := domain.DLQPolicy{MaxAttempts: 100, BaseMs: 1000, Multiplier: 10, MaxMs: 5000}
	if got := domain.NextRetry(3, p); got.DelayMs != 5000 {
