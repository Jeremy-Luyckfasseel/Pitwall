...
type ConsumerOptions struct {
	...
	RoutingKey     string // binding key (e.g. "lap.recorded")
...
// declares the durable queue, binds it on the routing key, and applies QoS.
// Story 1.9 will REDECLARE this queue with x-dead-letter-exchange args (RabbitMQ
...
func (b *Bus) DeclareConsumerQueue(opts ConsumerOptions) error {
	...
	if err := b.ch.QueueBind(opts.QueueName, opts.RoutingKey, opts.SourceExchange, false, nil); err != nil {
		return err
	}
