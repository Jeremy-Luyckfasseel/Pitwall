...
// keyed by its wire `type`, once at startup. On the Leaderboard it is used to
// validate-on-CONSUME (the mirror of Timing's validate-on-publish): an invalid
...
// CONTRACT_DIR (set explicitly in the container, where /contract is COPYed in)
// and otherwise walks up from the working directory to find it (dev/test).
...
// NewValidator compiles the schemas needed to validate the envelope, the
// heartbeat, and every event data payload. contractDir is the path to the
// /contract tree (see ResolveContractDir).
...
// its wire `type` (filename stem minus the trailing `.vN`). The envelope
// meta-schema is skipped (it is not a data payload).
...
// its data against the heartbeat data schema. A non-nil error means do not publish.
...
// ValidateEnvelopeBytes validates a marshalled message: the full envelope against
// the envelope schema, then its data against the schema registered for the
// envelope's `type`. It fails closed — an unknown type (no registered data
// schema) is an error. A non-nil error means: do not process; reject the message
// (validate-on-consume; CLAUDE.md rule 5 "invalid -> log + dead-letter").
