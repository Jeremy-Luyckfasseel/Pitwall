// Package heartbeat is Leaderboard's facade over the shared 1 s liveness emitter in
// libs/go-pitwall/heartbeat (Story 2.1). The emit loop + liveness touch-file mechanics
// live once in the library; this re-exports them so Leaderboard's main keeps
// constructing heartbeat.Emitter unchanged (it supplies its own Build/Validate/Publish).
package heartbeat
