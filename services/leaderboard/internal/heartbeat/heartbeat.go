...
// health). Dependencies are injected so the loop is unit-testable without a
// broker. Duplicated from services/timing/internal/heartbeat; libs extraction is
// Story 2.1.
package heartbeat
...
// Emitter ticks every Interval, building → validating → publishing a heartbeat
// and then touching LivenessFile. An invalid heartbeat is logged + dropped
// (never published) and does NOT touch the liveness file.
type Emitter struct {
...
// touch writes the timestamp to the liveness file, updating its mtime. The
// healthcheck treats a fresh mtime as proof the heartbeat loop (and thus the bus
// connection) is alive.
