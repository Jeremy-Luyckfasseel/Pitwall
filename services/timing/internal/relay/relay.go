// Package relay is the outbox publisher loop — the reliability spine (Story 1.4).
// It bridges the durable outbox (persistence) and the bus (messaging): it scans
// pending rows oldest-first, validates each against /contract, publishes to the
// service's own exchange, and marks a row sent ONLY after a confirmed broker ack.
//
// Failure handling is deliberate and split:
//   - validation failure  -> terminal: quarantine, never publish, never retry (AC2)
//   - broker unreachable   -> transient: keep pending, back off, retry forever (AC3)
//
// Our own valid events are never poison, so a transient failure is never parked
// or dropped; only an event we cannot validate is quarantined (a producer-side
// quarantine, distinct from the consumer-side RabbitMQ DLQ/parking, Story 1.9).
package relay
...
// Validator returns a non-nil error if a payload must NOT be published (it is
// then quarantined). Satisfied by (*messaging.Validator).ValidateEnvelopeBytes.
type Validator func(payload []byte) error

// Publisher publishes body to routingKey and returns nil ONLY on a confirmed
// broker ack. A nil error means the row may be marked sent. Satisfied by a
// confirm-mode channel's PublishConfirmed.
...
// New builds a relay with a buffered kick channel so a freshly-enqueued row can
// be published promptly without waiting a full poll interval.
...
// Run drains immediately, then on every poll tick and on every kick, until ctx
// is cancelled. It backs off (capped exponential) while the broker is
// unreachable and resets to the base interval once a drain succeeds.
...
// DrainOnce processes one batch of pending rows. It returns the number marked
// sent and a non-nil error if a transient publish failure occurred (the caller
// backs off). Validation failures are terminal and do NOT surface as an error —
// they are quarantined and the drain continues with the next row.
...
// Flush is a bounded best-effort drain for graceful shutdown (AC4). It attempts
// one batch and reports how many were sent and how many remain pending; whatever
// remains stays durably in the outbox for the next start (no loss either way).
