...
> local **`transponder_map`** (`checkInMethod:"transponder"`), whose latest-wins upsert
> SQL dates to Story 2.3. `TransponderStore.Assign` is what Story 2.4 adds: the
> **hand-out trigger** (FR33) that binds a transponder to a register-first-resolved
> `masterId` and reports whether the hand-out CHANGED an existing mapping, so the caller
> can log it distinctly (the simulator's `Prepare` logs a plain hand-out at info, a
