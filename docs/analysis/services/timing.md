...
lap times and detect personal records, and own the transponder→userId mapping and the
development **simulator**.

## System of record (owns)
- Raw scan events (gate + line) and computed lap times.
- **Transponder (hardware-id) → `userId`** mapping (assigned when a transponder is
  handed out at check-in).
- Local copies (ECST) of: `users` (keyed on `userId`, for scan validation), each
...
QR codes embed the `userId` (read directly, no lookup). Transponders carry a hardware
...
beaten it publishes `personal_record.broken {userId, sessionId, lapTimeMs,
...
| Scan of an unknown token (no `userId`) | Treat as walk-in: kiosk/counter path → `identity.lookup_requested`; until resolved, buffer the scan against the raw token, then attach `userId`. |
