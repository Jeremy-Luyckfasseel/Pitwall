...
> [Q&A](../00-questions-and-answers.md) Round 4,
> [ADR-0004](../../adr/0004-bus-only-health-and-self-ping.md). Conforms to the
...
- **Erasure:** consumes every service's `privacy.erased {userId, service, mode}` and tracks
...
- **Export:** consumes each service's `privacy.data_provided` slice, **assembles** the
  bundle, and emits `privacy.export_ready {userId, documentRef}` for Mailing to deliver.
