...
> The sole issuer of the canonical `userId`. Deliberately minimal. Decisions:
...
## Purpose
Guarantee that there is **exactly one canonical `userId` (UUID) per person**, so every
...
2. **Known** → return the existing `userId`. **Unknown** → mint a new UUID, store it.
3. Reply `identity.resolved {requestId, email, userId}`.

There is **no `isNew` flag** — the new-vs-returning decision lives entirely inside
Identity; every consumer simply **idempotently upserts** on the returned `userId`.
This is event-driven (no synchronous API), so it works within the bus-only rule.

The `userId` is embedded into the **QR code** issued to the user, so Timing reads it
...
- **Online registration**: Frontend stores credentials locally, publishes
  `identity.lookup_requested {email}` → `identity.resolved {userId}` → Frontend links
  the credential record; CRM/Driver upsert their records on the same `userId`.
- **Walk-in**: counter/kiosk publishes `identity.lookup_requested {email}` → mint →
  QR printed with the `userId`.

## Sad-path table
| Scenario | Handled outcome |
|---|---|
| Two lookups for the same new email race | Idempotent on email: the first mints, the second returns the same `userId`. Single-writer/unique-constraint on email guarantees one UUID. |
| Lookup for an email that already exists | Returns the existing `userId` (returning person) — no duplicate identity. |
